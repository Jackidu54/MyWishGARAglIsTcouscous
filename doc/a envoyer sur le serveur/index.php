...
$app->post('/liste/message/:id', function ($id) {
    ...
    $app = Slim\Slim::getInstance();
    $app->redirect(ControleurUrl::urlId('affiche_1_liste', $id));
    }else{
        \Slim\Slim::getInstance()->redirect(ControleurUrl::urlName('connection'));
    }
})->name('creer_message');
...
$app->get('/partage/:id', function ($id) {
    if(ControleurUser::verifPartage($id)){
        ...
        if(isset($_SESSION['partage']) && $_SESSION['partage']==$id && isset($_SESSION['email'])){
            ...
            $app = Slim\Slim::getInstance();
            $app->redirect(ControleurUrl::urlId('connection_partage', $id));
        }
    }else{
        \Slim\Slim::getInstance()->redirect(ControleurUrl::urlName('connection'));
    }
...
$app->post('/partage/inscription/:id', function ($id) {
    if($_SESSION['tokenInvite']){
        ...
        $app = Slim\Slim::getInstance();
        $app->redirect(ControleurUrl::urlId('afficher_liste_partagee', $id));
    }else{
...
$app->post('/item/creer/:id', function ($id) {
    if(isset($_SESSION['profile'])){
        ...
        if(isset($_SESSION['erreur']['tarifItem'])){
            $app = Slim\Slim::getInstance();
            $app->redirect(ControleurUrl::urlId('createur_item', $id));
        }else {
...
$app->post('/item/delete/:id', function ($id) {
    ...
    $app->redirect(ControleurUrl::urlId('affiche_1_liste', $app->request->post('listeid')));
    }else{
        \Slim\Slim::getInstance()->redirect(ControleurUrl::urlName('connection'));
    }
})->name('delete_item');
...
$app->post('/user/create', function() {
    ...
    if($rep){
        \Slim\Slim::getInstance()->redirect(ControleurUrl::urlName('affiche_listes'));
    }else {
        \Slim\Slim::getInstance()->redirect(ControleurUrl::urlName('inscription'));
    }
})->name('creer_user');
...
$app->post('/item/reserve/:id', function ($id) {
    ...
    \Slim\Slim::getInstance()->redirect(ControleurUrl::urlId('afficher_liste_partagee', $_SESSION['partage']));
})->name('reserve_item');
